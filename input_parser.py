...
def parse_input(st_db):
    ...
    while not re.match(option_regex,user_input): 
        print("Error: incorrect command given. Please choose 'a', 'c' 'r' or 'g' as a command.", file = sys.stderr)
        user_input = input()
...
def parse_a(user_input):
    ...
    while not re.match(a_regex,user_input):
        print("Error: You have not formatted your command correct. Please try again with the following format: a \"<STREET NAME>\" (X1,Y1) (X2,Y2)..(Xn,Yn)", file = sys.stderr)
...
def parse_c(user_input):
    ...
    while not re.match(c_regex,user_input):
        print("Error: You have not formatted your command correct. Please try again with the following format: c \"<STREET NAME>\" (X1,Y1) (X2,Y2)..(Xn,Yn)", file = sys.stderr)
...
def parse_r(user_input):
    ...
    while not re.match(r_regex,user_input):
        print("Error: You have not formatted your command correctly. Please try again with the following format: r \"<STREET NAME>\"", file = sys.stderr)
...
def parse_g(user_input):
    ...
    while not re.match(g_regex,user_input):
        print("Error: You have not formatted your command correctly. Please enter only 'g' to generate the graph", file = sys.stderr)
...
def main():
    ...
    st_db = street_db.Street_Database()
